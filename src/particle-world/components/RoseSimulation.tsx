...
class Particle {
  ...
  update(time: number, dt: number, config: RoseConfig, noise: (x: number, y: number) => number) {
  ...
  draw(ctx: CanvasRenderingContext2D, spriteSheet: HTMLCanvasElement, time: number, config: RoseConfig) {
...
export const RoseSimulation: React.FC<{
  config: RoseConfig;
  ...
  const generateSprites = useCallback(() => {
    ...
    for (let i = 0; i < SPRITE_COUNT; i++) {
      ...
      const hue = (345 + Math.random() * 25) % 360;
      const sat = 75 + Math.random() * 25;
      const lum = 35 + Math.random() * 25;
      sCtx.fillStyle = `hsl(${hue}, ${sat}%, ${lum}%)`;

      sCtx.beginPath();
      sCtx.moveTo(0, -r);
      sCtx.bezierCurveTo(r * 1.5, -r * 0.5, r * 1.2, r * 1.2, 0, r * 1.5);
      sCtx.bezierCurveTo(-r * 1.2, r * 1.2, -r * 1.5, -r * 0.5, 0, -r);
      sCtx.fill();

      sCtx.strokeStyle = `hsla(${hue}, ${sat}%, ${Math.max(10, lum - 20)}%, 0.6)`;
      sCtx.lineWidth = 1;
      sCtx.beginPath();
      sCtx.moveTo(0, -r * 0.5);
      sCtx.lineTo(0, r);
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      gradient.addColorStop(0, '#120a0d');
      gradient.addColorStop(1, '#210d14');
